...
## 2026-09-04 통합 업무 알림 및 v0.3.65 공개 배포
...
- GitHub Windows release 작업 `33832156491`이 성공했다. 공개 릴리스에는 버전 설치 파일, 고정 이름 설치 파일, 자동 업데이트 ZIP, SHA-256 목록이 모두 게시됐고 `releases/latest`가 `v0.3.65` 설치 파일로 연결되는 것을 확인했다. 공개 설치 파일의 SHA-256은 `47C04034286EF880B70F2997B3AA33196F0ED60CEA2E0616D17CFAD0330C884D`다.

## 2026-09-04 업무카드 상세·드래그 개선 검토본

- 나의 공간과 직원업무의 업무카드를 공통 단일 카드 디자인으로 바꿨다. 줄별 외곽선과 별도 상세보기 박스를 제거하고 업무 유형·프로젝트·업무명·상태·마감일을 한 카드에 표시한다. 카드 전체의 더블클릭으로 세 업무 유형 모두 공통 상세창을 열며, 체크리스트 이동과 본인 소유 업무 수정 동선을 제공한다.
- 카드 자체가 마우스 입력을 처리해 반투명 카드 미리보기가 커서를 따라가고 원래 위치는 비워 둔다. 직원업무 드롭 영역은 대상 카드와 삽입 위치를 강조하며 150ms 간격 애니메이션을 사용한다. 관리자 순서 변경은 해당 직원 개인 우선순위에 저장되고, 다른 직원에게 놓으면 담당자와 양쪽 순서를 원자적으로 갱신한다. 사내 업무의 직원 간 이관은 UI와 서버에서 모두 차단한다.
- 운영 Supabase에 직원 우선순위 조회와 위치 지정 이동 RPC를 적용했다. 관리자·활성 소속·동시 수정 버전을 검사하며 담당자 이관과 신규/이전 담당자 영구 알림 2건을 같은 트랜잭션으로 처리한다. 롤백 검사에서 일반 직원 차단, 이전 담당자 우선순위 잔존 없음, 양쪽 알림 생성을 확인했다.
- Teams 전체 Pytest 186건, Python compileall, 공백 검사와 Web Vitest 37건·Svelte check 0 오류/0 경고를 통과했다. 오프스크린 카드 렌더를 확인했고 검토 EXE(SHA-256 `474CA8ACC8963FA5DCF7CB77CE4AA3E8B81233B8EFDA8B9DCBDD81F70DF0A7AB`)는 격리 환경에서 health `ok`를 기록했다. 설치 파일 SHA-256은 `7BAC7261FF2C8D463E7BF1EC7CE5A1B7CB9C0D6E94ED6987148CAA9DA50619B2`다. 공개 업데이트는 만들지 않았다.

## 2026-09-04 v0.3.66 공개 배포
