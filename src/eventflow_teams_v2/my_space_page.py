...
class MySpacePage(QWidget):
    ...
    def refresh(self) -> None:
        ...
        tasks = self.db.query("""SELECT w.remote_id,w.event_id,w.work_scope,w.major,w.name,w.status,w.due_date,w.sort_order,COALESCE(e.name,CASE WHEN w.work_scope='COMPANY' THEN '회사 공통' ELSE '프로젝트' END) event_name FROM teams_v3_work_items w LEFT JOIN teams_v2_entity_map map ON map.entity_type='EVENT' AND map.remote_id=w.event_id LEFT JOIN events e ON e.id=map.local_id WHERE w.assigned_member_user_id=? AND w.is_removed=0 AND w.status NOT IN ('완료','해당없음') ORDER BY COALESCE(w.due_date,'9999-12-31'),w.sort_order""", (self.user_id,))
